...
| `glossary.html` | Plain-language definitions of the environmental-law, heritage and policy terms used across the site (added 14 Sep 2026, replacing the term list that used to live inline in `briefing.html` before it was trimmed) |
...
- **Identical nav on every page.** A `.topbar`/`.navlinks` bar plus a matching footer `<nav>` repeat on all eight pages, with `aria-current="page"` marking the current page (and, on a page's own footer nav, that page is simply omitted rather than shown as a dead self-link — match that pattern for any new page). Adding a page means adding it to *both* the topbar and footer nav blocks on *every existing page*, not just the new one.
...
- **Typography is deliberately neutral** (set 14 Sep 2026, replacing a Spectral + Libre Franklin pairing that read too editorial). **Source Serif 4** — falling back to Georgia — for both headings and body; **IBM Plex Mono** only for small labels (eyebrows, nav, `.mast-meta`, tags, `dl.factlist` labels). Headings are small on purpose: page titles `clamp(1.6rem,3.6vw,2.05rem)`, `h2.sec` 1.2rem, `h3.sub` 1.03rem. The target is a government/NGO information sheet, not a magazine — if a change would make a heading bigger or add a second display face, that's working against the brief. Note `the-record.html` and `glossary.html` carry their own `<style>` blocks that re-declare some of these sizes; change both places or they drift.
- **`dl.factlist`** is the house pattern for information-sheet sections: a `dt` label column (mono, uppercase, small) beside a `dd` fact column, collapsing to one column under 560px. Prefer it over a bullet list whenever content is genuinely label/value — `briefing.html`'s "What is being proposed" is the reference example.
...
- **WhatsApp voice-note line** — live as of 15 Sep 2026. `have-your-say.html` links `wa.me/27630169518`, a dedicated line ("Hopefield Origins" in WhatsApp, not Anna's personal number).
...
- **Afrikaans translations** — drafts are live and toggleable on every page now (`the-record.html` and `sources.html` caught up 13 Sep 2026). `briefing.html` (the 13 Sep draft) had its first native-speaker review pass (Ronelle, ~17 Sep 2026) — 73 PDF-comment corrections applied, plus the same recurring errors/word-choice fixes swept across the other pages wherever they happened to recur (`the-record.html`, `sources.html`, `glossary.html`). **Corrected 19 Sep 2026:** `briefing.html`'s `.draftbar` had kept its generic "please have someone check this" text even after Ronelle's review was applied — a straightforward oversight (the corrections commit didn't touch the banner), not a deliberate wait-for-more-review state. It now reads `.draftbar.reviewed` (a new green variant, `style.css`) with text naming the review date instead of asking for one. **The other seven pages have not had a full native-speaker review** and correctly still carry the original ochre `.draftbar` asking for one — only the specific errors known from Ronelle's `briefing.html` pass were fixed where they happened to reappear in them; anything not covered by that pass is still unreviewed. No fixed reviewer for the remaining pages — likely to be different volunteers over time, or Anna herself if she has time. Don't assume a review is scheduled for the rest. When any of those seven gets its own full review, give it the same `.draftbar.reviewed` treatment rather than leaving the stale banner up. PDF review copies (see below) exist for all eight pages, to make that review easier to do offline/by email — regenerate a page's copy before sending if the page has changed since the PDF's date.
...
- **Cloudflare Web Analytics** (added 12 Sep 2026, chosen specifically for being cookie-less and aggregate-only, in keeping with the site's minimal/low-bandwidth design) — injected from `site.js`, not hardcoded per-page, so the beacon token only needs to live in one place. The injection guards itself against the placeholder value (`cfToken.indexOf("REPLACE_WITH") !== 0`), so it is a safe no-op until a real token from the Cloudflare dashboard (Analytics & Logs → Web Analytics → add this hostname, no DNS change needed) is substituted in. `privacy.html`'s "What is collected" and "Why, and on what basis" sections were updated in the same change that added this — if analytics setup ever changes (a different tool, more detail, cookies), update `privacy.html` in that same change too, not after. `have-your-say.html`'s "this survey collects nothing about you that you do not enter yourself" line was deliberately left as-is — it's scoped to the survey specifically and remains true regardless of site-wide aggregate analytics.
