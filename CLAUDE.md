...
- **Afrikaans translations** — drafts are live and toggleable on every page now (`the-record.html` and `sources.html` caught up 13 Sep 2026). `briefing.html` (the 13 Sep draft) had its first native-speaker review pass (Ronelle, ~17 Sep 2026) — 73 PDF-comment corrections applied, plus the same recurring errors/word-choice fixes swept across the other pages wherever they happened to recur (`the-record.html`, `sources.html`, `glossary.html`). It still carries the `.draftbar` banner. **The other pages have not had a full native-speaker review** — only the specific errors known from Ronelle's `briefing.html` pass were fixed where they happened to reappear in them; anything not covered by that pass is still unreviewed. No fixed reviewer for the remaining pages — likely to be different volunteers over time, or Anna herself if she has time. Don't assume a review is scheduled for the rest. PDF review copies (see below) exist for all eight pages, to make that review easier to do offline/by email.
